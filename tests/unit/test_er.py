...
def test_same_block_shared_predicate() -> None:
    pair = MentionPair(
        id_a=_A, id_b=_B,
        predicate_a="road_damage", predicate_b="road_damage",
        text_a="pothole on Egnatia", text_b="pot hole eg nat ia",
...
def test_same_block_high_text_overlap() -> None:
    pair = MentionPair(
        id_a=_A, id_b=_B,
        predicate_a="road_damage", predicate_b="water_leak",
...
def test_features_same_predicate_same_text() -> None:
    pair = MentionPair(
        id_a=_A, id_b=_B,
        predicate_a="road_damage", predicate_b="road_damage",
        text_a="pothole on Egnatia", text_b="pothole on Egnatia",
        lat_a=40.64, lon_a=22.94, lat_b=40.64, lon_b=22.94,
    )
...
def test_features_different_predicates() -> None:
    pair = MentionPair(
        id_a=_A, id_b=_B,
        predicate_a="road_damage", predicate_b="water_leak",
        text_a="pothole", text_b="pipe burst",
        lat_a=40.64, lon_a=22.94, lat_b=40.65, lon_b=22.95,
    )
    fv = compute_features(pair)
...
def test_features_both_no_location() -> None:
    pair = MentionPair(
        id_a=_A, id_b=_B,
        predicate_a="noise", predicate_b="noise",
        text_a="loud music at night", text_b="loud music at night",
        lat_a=None, lon_a=None, lat_b=None, lon_b=None,
    )
    fv = compute_features(pair)
...
def test_features_one_location_missing() -> None:
    pair = MentionPair(
        id_a=_A, id_b=_B,
        predicate_a="road_damage", predicate_b="road_damage",
        text_a="pothole", text_b="pothole",
        lat_a=40.64, lon_a=22.94, lat_b=None, lon_b=None,
    )
    fv = compute_features(pair)
...
def test_er_decision_clear_match() -> None:
    ...
    fv = FeatureVector(same_predicate=1.0, text_similarity=1.0,
                       geo_proximity=1.0, combined=1.0)
    verdict, _score = er_decision(fv)  # type: ignore[arg-type]
...
def test_er_decision_clear_non_match() -> None:
    ...
    fv = FeatureVector(same_predicate=0.0, text_similarity=0.0,
                       geo_proximity=0.0, combined=0.0)
    verdict, _score = er_decision(fv)  # type: ignore[arg-type]
...
def test_er_decision_uncertain() -> None:
    ...
    fv = FeatureVector(same_predicate=1.0, text_similarity=0.3,
                       geo_proximity=0.0, combined=0.50)
